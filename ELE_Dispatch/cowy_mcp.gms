...
*lambda_dem
eq_demcon(h).. sum((s,f,pid)$v(s,f,pid,h), X(s,f,pid,h)) =e= d(h) ; 
...
positive variable lambda_cap ;

variable lambda_dem ; 

zpc_x(s,f,pid,h)$v(s,f,pid,h).. 
                lambda_cap(s,f,pid,h) 
                + c_v(s,f,pid) 
                + fcost(f) * hr(s,f,pid)
                - lambda_dem(h)
                =g=
...
model cowy_mcp
/
eq_demcon.lambda_dem,
eq_capcon.lambda_cap,
zpc_x.x
/;

lambda_dem.l(h) = eq_demcon.m(h) ; 
lambda_cap.l(s,f,pid,h)$v(s,f,pid,h) = eq_capcon.m(s,f,pid,h) ; 

cowy_mcp.iterlim = 0 ;

solve cowy_mcp using mcp ; 
