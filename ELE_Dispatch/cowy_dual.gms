...
equation zpc_x(s,f,pid,h) ; 

positive variable lambda_cap, lambda_dem ; 

zpc_x(s,f,pid,h)$v(s,f,pid,h).. 
                lambda_cap(s,f,pid,h) + c_v(s,f,pid) + fcost(f) * hr(s,f,pid)
                =g=
                lambda_dem(h) ; 

equation eq_dual_obj ; 
variable dual_obj ; 

eq_dual_obj.. dual_obj =e= sum(h,lambda_dem(h) * d(h)) - sum((s,f,pid,h)$v(s,f,pid,h),lambda_cap(s,f,pid,h) * cf(f,h) * cap(s,f,pid)) ;

model cowy_dual /zpc_x, eq_dual_obj/ ; 

solve cowy_dual using lp maximizing dual_obj ; 
