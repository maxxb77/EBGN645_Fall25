...
parameter p_bin, q_bin ; 
q_bin(bin,h) = d(h) / 50 ; 
p_bin(bin,h) = 0 ; 

positive variable q_v(bin,h) ; 

eq_cost.. Z =e= 
    sum((s,f,pid,h)$v(s,f,pid,h),X(s,f,pid,h)*(c_v(s,f,pid) + fcost(f) * hr(s,f,pid)))
    - sum((bin,h),p_bin(bin,h) * q_v(bin,h))$sw_elas  ; 

eq_capcon(s,f,pid,h)$v(s,f,pid,h).. cf(f,h) * cap(s,f,pid) =g= X(s,f,pid,h) ; 

eq_demcon(h).. sum((s,f,pid)$v(s,f,pid,h), X(s,f,pid,h)) 
                =g= 
               d(h)$(not sw_elas) + sum(bin,q_v(bin,h))$sw_elas; 

equation eq_dembinlim(bin,h) ; 

eq_dembinlim(bin,h)$sw_elas..
    q_bin(bin,h) =g= q_v(bin,h) ; 
...
rep("BAU",s,f,pid,h)$v(s,f,pid,h) = 1e-5 + x.l(s,f,pid,h) ; 


alias(bin,bin2) ; 
p_bin(bin,h) = eq_demcon.m(h) * (sum(bin2$(ord(bin2)<=ord(bin)), q_bin(bin2,h)) / d(h)) ** (1/elas) ; 
sw_elas = 1 ; 
solve cowy using lp minimizing z ; 
rep("BAU_elas",s,f,pid,h)$v(s,f,pid,h) = 1e-5 + x.l(s,f,pid,h) ; 

parameter check_dem ; 
check_dem(h) = d(h) / sum(bin,q_v.l(bin,h)) ; 
...
sw_cap = 1 ; 
solve cowy using lp minimizing z ; 
rep("CAP",s,f,pid,h)$v(s,f,pid,h) = 1e-5 + x.l(s,f,pid,h) ; 

sw_cap = 0 ; 
sw_tps = 1 ; 

solve cowy using lp minimizing z ; 
rep("TPS",s,f,pid,h)$v(s,f,pid,h) = 1e-5 + x.l(s,f,pid,h) ; 

execute_unload 'cowy_elas.gdx' ; 
